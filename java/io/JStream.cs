...
namespace SSFTP.SharpSsh.java.io
{
	/// <summary>
	...
	public class JStream : IO.Stream
	{
		...
		public long skip(long len)
		{
			...
			while(len>0)
			{
				i=Read(buf, count, (int)len);//SSFTP: possible lost of pressision
                if (i<=0)
				{
		...
		{
			if(s is SSFTP.Streams.PipedInputStream)
			{
				return ((SSFTP.Streams.PipedInputStream)s).available();
			}
